...
const webDistDir =
  process.env.PLAYWRIGHT_WEB_DIST_DIR ?? path.join(".next-playwright", webPort.toString());
const apiBaseUrl =
  process.env.AGRO_E2E_API_BASE_URL ?? `http://127.0.0.1:${apiPort}`;
const baseUrl =
  process.env.PLAYWRIGHT_BASE_URL ?? `http://127.0.0.1:${webPort}`;
const useProdWebServer = process.env.PLAYWRIGHT_USE_PROD_WEB_SERVER === "1";
...
const webServerCommand = useProdWebServer
  ? `corepack pnpm build && corepack pnpm exec next start --hostname 127.0.0.1 --port ${webPort}`
  : `rm -rf "${webDistDir}" && corepack pnpm exec next dev --hostname 127.0.0.1 --port ${webPort}`;
