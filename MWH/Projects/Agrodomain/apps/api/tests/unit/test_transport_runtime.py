...
def test_transport_runtime_blocks_checkpoint_before_transit(session) -> None:
    repository = TransportRepository(session)
    runtime = TransportRuntime(repository)
    load = repository.create_load(
        load_id="load-003",
    ...
    assigned = runtime.assign_load(
        ...
        actor_role="transporter",
        load=load,
        vehicle_info={"plate_number": "AS-3002-26"},
        location_lat=9.4075,
        location_lng=-0.8533,
        notes="Assigned",
    )
    ...
    with pytest.raises(CommandRejectedError) as exc:
        runtime.log_operational_event(
            ...
            location_lat=8.0,
            location_lng=-0.5,
            notes="Premature checkpoint",
        )

    assert exc.value.reason_code == "transport_checkpoint_requires_in_transit"


def test_transport_runtime_blocks_duplicate_delivery(session) -> None:
    repository = TransportRepository(session)
    runtime = TransportRuntime(repository)
    load = repository.create_load(
        load_id="load-004",
    ...
    assigned = runtime.assign_load(
        ...
        actor_role="transporter",
        load=load,
        vehicle_info={"plate_number": "AS-3003-26"},
        location_lat=9.4075,
        location_lng=-0.8533,
        notes="Assigned",
    )
    delivered = runtime.deliver_shipment(
        ...
        load=assigned.load,
        shipment=assigned.shipment,
        proof_of_delivery_url="https://cdn.example.com/proof/load-004.jpg",
        location_lat=5.6037,
        location_lng=-0.187,
        notes="Receiver signed",
    )
    ...
    with pytest.raises(CommandRejectedError) as exc:
        runtime.deliver_shipment(
            actor_id="actor-transporter-gh-yaw",
            actor_role="transporter",
            load=delivered.load,
            shipment=delivered.shipment,
            proof_of_delivery_url="https://cdn.example.com/proof/load-004-second.jpg",
            location_lat=5.6037,
            location_lng=-0.187,
            notes="Second proof attempt",
        )

    assert exc.value.reason_code == "transport_shipment_already_delivered"


def test_transport_runtime_dispatch_reassigns_shipment(session) -> None:
    repository = TransportRepository(session)
    runtime = TransportRuntime(repository)
    load = repository.create_load(
        load_id="load-003",
    ...
    assigned = runtime.dispatch_assign_load(
        dispatcher_actor_id="actor-cooperative-gh-ops",
        dispatcher_role="cooperative",
        transporter_actor_id="actor-transporter-gh-yaw",
        load=load,
        vehicle_info={"plate_number": "AS-3001-26", "vehicle_type": "flatbed"},
        location_lat=9.4075,
        location_lng=-0.8533,
        notes="Assigned from dispatch board",
    )

    reassigned = runtime.reassign_shipment(
        actor_id="actor-cooperative-gh-ops",
        actor_role="cooperative",
        load=assigned.load,
        shipment=assigned.shipment,
        transporter_actor_id="actor-transporter-gh-kojo",
        vehicle_info={"plate_number": "GT-2222-26", "vehicle_type": "flatbed"},
        location_lat=8.0,
        location_lng=-1.0,
        notes="Swap to a closer carrier",
    )
    session.commit()

    assert reassigned.load.assigned_transporter_actor_id == "actor-transporter-gh-kojo"
    assert reassigned.shipment.transporter_actor_id == "actor-transporter-gh-kojo"
    assert reassigned.event.event_type == "reassigned"
