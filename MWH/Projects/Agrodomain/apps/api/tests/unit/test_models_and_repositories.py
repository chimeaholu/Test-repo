...
def test_marketplace_repository_uses_listing_snapshot_when_new_publish_revision_is_missing(session) -> None:
    repository = MarketplaceRepository(session)
    listing = repository.create_listing(
        listing_id="listing-003c",
        actor_id="actor-farmer-gh-ama",
        country_code="GH",
        title="Fresh millet lot",
        commodity="Millet",
        quantity_tons=4.8,
        price_amount=410,
        price_currency="GHS",
        location="Bolgatanga, GH",
        summary="Fresh millet lot with warehouse receipt and pickup schedule attached.",
    )
    repository.publish_listing(listing=listing)
    session.flush()
    session.execute(
    ...
    buyer_projection = repository.get_published_listing(listing_id="listing-003c", country_code="GH")

    assert buyer_projection is not None
    assert buyer_projection.title == "Fresh millet lot"
    assert buyer_projection.revision_number == 2
    assert buyer_projection.view_scope == "buyer_safe"


def test_marketplace_repository_preserves_buyer_safe_revision_when_published_revision_is_missing(session) -> None:
    repository = MarketplaceRepository(session)
    listing = repository.create_listing(
        listing_id="listing-003d",
        actor_id="actor-farmer-gh-ama",
        country_code="GH",
        title="Export sorghum lot",
        commodity="Sorghum",
        quantity_tons=5.2,
        price_amount=470,
        price_currency="GHS",
        location="Tamale, GH",
        summary="Export sorghum lot with verified storage proof and load plan attached.",
    )
    repository.publish_listing(listing=listing)
    repository.update_listing(
        listing=listing,
        title="Export sorghum lot revised",
        commodity="Sorghum",
        quantity_tons=5.9,
        price_amount=495,
        price_currency="GHS",
        location="Tamale consolidation yard, GH",
        summary="Revised sorghum lot with unpublished logistics changes.",
    )
    session.flush()
    session.execute(
        delete(ListingRevision).where(
    ...
    buyer_projection = repository.get_published_listing(listing_id="listing-003d", country_code="GH")

    assert buyer_projection is not None
    assert buyer_projection.title == "Export sorghum lot"
    assert buyer_projection.location == "Tamale, GH"
    assert buyer_projection.revision_number == 2
    assert buyer_projection.view_scope == "buyer_safe"
...
def test_transport_repository_persists_load_shipment_and_events(session) -> None:
    repository = TransportRepository(session)
    load = repository.create_load(
        load_id="load-transport-001",
    ...
    repository.update_load(
        load=load,
        status="assigned",
        assigned_transporter_actor_id="actor-transporter-gh-yaw",
    )
    shipment = repository.create_shipment(
        shipment_id="shipment-transport-001",
        load_id=load.load_id,
        transporter_actor_id="actor-transporter-gh-yaw",
        country_code="GH",
        vehicle_info={"plate_number": "AS-1001-26"},
        current_location_lat=9.4075,
        current_location_lng=-0.8533,
    )
    repository.create_shipment_event(
        event_id="event-transport-001",
        ...
        location_lat=9.4075,
        location_lng=-0.8533,
        notes="Truck assigned",
    )
    session.commit()

    stored_load = session.execute(
        select(TransportLoad).where(TransportLoad.load_id == "load-transport-001")
    ).scalar_one()
    stored_shipment = session.execute(
        select(Shipment).where(Shipment.shipment_id == "shipment-transport-001")
    ).scalar_one()
    stored_events = session.execute(
        select(ShipmentEvent).where(ShipmentEvent.shipment_id == "shipment-transport-001")
    ).scalars().all()

    assert stored_load.assigned_transporter_actor_id == "actor-transporter-gh-yaw"
    assert stored_shipment.status == "assigned"
    assert stored_events[0].event_type == "assigned"
