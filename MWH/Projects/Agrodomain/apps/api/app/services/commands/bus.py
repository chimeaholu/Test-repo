...
class CommandBus:
    def __init__(
        ...
        *,
        ...
        self.fund_repository = FundRepository(session)
    ...
    def dispatch(
        self, envelope: CommandEnvelope, auth_context: AuthContext
        ...
        if (
            ...
            or envelope.command_name.startswith("fund.")
        ...
        elif "investment" in result:
            investment_payload = cast(dict[str, object], result["investment"])
            aggregate_id = str(investment_payload["investment_id"])
            aggregate_type = "investment"
        elif "opportunity" in result:
            opportunity_payload = cast(dict[str, object], result["opportunity"])
            aggregate_id = str(opportunity_payload["opportunity_id"])
            aggregate_type = "funding_opportunity"
        ...
        if mrv_evidence is not None:
            ...
            self.audit_repository.record_event(
                ...
                reason_code=(
                    str(cast(list[object], mrv_evidence["degraded_reason_codes"])[0])
                    if mrv_evidence.get("degraded_reason_codes")
                    else None
                ),
                schema_version=envelope.metadata.schema_version,
                idempotency_key=envelope.metadata.idempotency_key,
