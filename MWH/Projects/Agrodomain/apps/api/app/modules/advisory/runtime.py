...
class AdvisoryRuntime:
    ...
    def submit_request(
        ...
        *,
        ...
        raw_sensitive_topics = policy_context.get("sensitive_topics", [])
        sensitive_topics: list[str] = []
        if isinstance(raw_sensitive_topics, list):
            sensitive_topics = [
                str(item).strip().lower() for item in raw_sensitive_topics if str(item).strip()
            ]
...
def _policy_sensitive_from_context(policy_context: dict[str, object]) -> bool:
    raw_topics = policy_context.get("sensitive_topics", [])
    topics: list[str] = []
