...
describe("negotiation inbox", () => {
  ...
  it("submits a buyer offer and renders recent activity details", async () => {
    ...
    mockMarketplaceApi.listNegotiations
    ...
    mockMarketplaceApi.createNegotiation.mockResolvedValue({
      data: {
    ...
    mockMarketplaceApi.getNegotiationThread.mockResolvedValue({ data: createdThread });
    ...
    await waitFor(() => {
      expect(mockMarketplaceApi.createNegotiation).toHaveBeenCalledWith(
  ...
  it("renders pending confirmation controls only for the authorized confirmer", async () => {
    ...
    mockMarketplaceApi.getNegotiationThread.mockResolvedValue({ data: thread });
  ...
  it("surfaces inaccessible thread handling when the selected thread is unavailable", async () => {
    ...
    mockMarketplaceApi.getNegotiationThread.mockRejectedValue(new Error("thread_not_found"));
    ...
    expect(screen.getByText("This conversation is no longer available to you or may have been removed.")).toBeInTheDocument();
  });
