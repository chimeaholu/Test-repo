...
describe("route guards", () => {
  it("allows a consented actor to open their required advisor route directly", () => {
    const session = {
      ...consentGrantedSession,
      actor: {
        ...consentGrantedSession.actor,
        role: "advisor" as const,
        membership: {
          ...consentGrantedSession.actor.membership,
          role: "advisor" as const,
        },
      },
      available_roles: ["advisor" as const],
    };
    expect(getRouteDecision("/app/advisor/requests", session)).toEqual({
      allowed: true,
      redirectTo: null,
      reason: "ok",
    });
  ...
  it("allows a consented admin to open analytics directly", () => {
    const session = {
      ...consentGrantedSession,
      actor: {
        ...consentGrantedSession.actor,
      ...
      available_roles: ["admin" as const],
    };
    expect(getRouteDecision("/app/admin/analytics", session)).toEqual({
      allowed: true,
      redirectTo: null,
      reason: "ok",
    });
