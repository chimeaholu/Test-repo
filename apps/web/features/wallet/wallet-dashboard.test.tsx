...
describe("wallet dashboard", () => {
  beforeEach(() => {
    ...
    mockAgroApiClient.getWalletSummary.mockResolvedValue({ data: buildWalletSummary() });
    mockAgroApiClient.listWalletTransactions.mockResolvedValue({
      data: {
        schema_version: "2026-04-18.wave1",
    ...
    mockAgroApiClient.listEscrows.mockResolvedValue({
      data: {
        schema_version: "2026-04-18.wave1",
        items: [buildEscrow()],
      },
  ...
  it("renders partner-pending fallback surfaces with outbox guidance", async () => {
    ...
    expect(await screen.findByText("Pending fallback active")).toBeInTheDocument();
    expect(screen.getByText("Settlement fallback is visible and recoverable")).toBeInTheDocument();
    expect(screen.getByRole("link", { name: "Open outbox" })).toHaveAttribute("href", "/app/offline/outbox");
  ...
  it("submits a replay-safe funding retry and renders request evidence", async () => {
    mockAgroApiClient.fundEscrow.mockResolvedValue({
      data: {
        escrow: buildEscrow({
          state: "funded",
    ...
    await waitFor(() => {
      expect(mockAgroApiClient.fundEscrow).toHaveBeenCalledWith(
        {
          escrow_id: "escrow-1",
          note: "Delivery proof checked against the latest canonical timeline.",
    ...
    expect(await screen.findByText("Replay-safe command receipt")).toBeInTheDocument();
    expect(screen.getByText("Replayed single effect")).toBeInTheDocument();
    expect(screen.getByText("req-funded")).toBeInTheDocument();
    expect(screen.getByText("idem-funded")).toBeInTheDocument();

    const evidenceCard = screen.getByText("Replay-safe command receipt").closest("section");
    expect(evidenceCard).toHaveTextContent("Audit events");
    expect(evidenceCard).toHaveTextContent("Settlement notifications");
  });
});
