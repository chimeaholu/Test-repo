...
def test_command_bus_rejects_internal_finance_approval_shortcut(session) -> None:
    schema_version = get_envelope_schema_version()
    settings = Settings(
        database_url="sqlite:///:memory:",
        api_tokens={"test-token": "system:test"},
    ...
    bus = CommandBus(
        ...
        telemetry=type(
            ...
            {
                "record_command": lambda *args, **kwargs: None,
                "record_finance_queue_state": lambda *args, **kwargs: None,
            },
        )(),
        correlation_id="corr-finance-shortcut",
        settings=settings,
    )
    ...
    create_envelope = CommandEnvelope.model_validate(
        {
            "metadata": {
                "request_id": str(uuid4()),
                "idempotency_key": "idem-fin-create",
                "actor_id": "system:test",
                "country_code": "GH",
                "channel": "pwa",
                "schema_version": schema_version,
    ...
    created = bus.dispatch(create_envelope, auth_context)
    finance_request_id = str(created.result["finance_request"]["finance_request_id"])

    decision_envelope = CommandEnvelope.model_validate(
        {
            "metadata": {
                "request_id": str(uuid4()),
                "idempotency_key": "idem-fin-decision",
                "actor_id": "system:test",
                "country_code": "GH",
                "channel": "pwa",
                "schema_version": schema_version,
                "correlation_id": "corr-finance-shortcut-2",
                "occurred_at": "2026-04-18T05:05:00+00:00",
                "traceability": {"journey_ids": ["CJ-004"], "data_check_ids": ["DI-003"]},
            },
            "command": {
                "name": "finance.partner_decisions.record",
                "aggregate_ref": finance_request_id,
                "mutation_scope": "regulated.finance",
                "payload": {
    ...
    try:
        bus.dispatch(decision_envelope, auth_context)
    except CommandRejectedError as exc:
        assert exc.status_code == 403
        assert exc.error_code == "partner_boundary_violation"
    else:
        raise AssertionError("internal finance approval shortcut should be rejected")
