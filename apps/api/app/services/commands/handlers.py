...
from app.db.repositories.traceability import (
    TraceMilestone,
    TraceabilityContinuityError,
...
class WorkflowCommandHandler:
    ...
    def _build_alert_from_observation(
        ...
        *,
        ...
        def precedence_rank(candidate: dict[str, object]) -> int:
            return cast(int, candidate["precedence_rank"])

        selected = sorted(candidates, key=precedence_rank)[0]
        selected_rank = precedence_rank(selected)
    ...
    def handle(self, envelope: CommandEnvelope, auth_context: AuthContext) -> dict[str, object]:
        ...
        if envelope.command_name == "finance.partner_decisions.record":
            ...
            finance_request_record = self.finance_repository.get_request(
                finance_request_id=finance_request_id
            )
            if finance_request_record is None:
                raise CommandRejectedError(
                    status_code=404,
            ...
            if outcome == "approved" and finance_request_record.responsibility_boundary.get(
            ...
            decision = self.finance_repository.create_decision(
                ...
                partner_reference_id=(
                    str(envelope.payload["partner_reference_id"])
                    if envelope.payload.get("partner_reference_id") is not None
                    else finance_request_record.partner_reference_id
                ),
                responsibility_boundary=finance_request_record.responsibility_boundary,
                policy_context=finance_request_record.policy_context,
                transcript_link=(
        ...
        if envelope.command_name == "traceability.events.append":
            ...
            consignment_record = self.traceability_repository.get_consignment(
                consignment_id=consignment_id
            )
            if consignment_record is None:
                raise CommandRejectedError(
                    status_code=404,
            ...
            if consignment_record.country_code != envelope.metadata.country_code:
                raise CommandRejectedError(
                    status_code=403,
            ...
            milestone = cast(TraceMilestone, str(envelope.payload.get("milestone", "")))
            if milestone not in {
                "harvested",
                "handoff_confirmed",
                "dispatched",
            ...
            try:
                event = self.traceability_repository.append_event(
                    consignment=consignment_record,
                    request_id=str(envelope.metadata.request_id),
                    idempotency_key=envelope.metadata.idempotency_key,
                    actor_id=envelope.metadata.actor_id,
                    actor_role=auth_context.role or "unknown",
                    country_code=envelope.metadata.country_code,
                    correlation_id=envelope.metadata.correlation_id,
            ...
            events = self.traceability_repository.list_events(
                consignment_id=consignment_record.consignment_id
            )
            return {
                "consignment": _consignment_to_payload(
                    consignment_record, envelope.metadata.schema_version
                ),
        ...
        if envelope.command_name == "market.negotiations.create":
            ...
            listing_id = str(envelope.payload["listing_id"])
            published_listing_record = self.marketplace_repository.get_published_listing(
                listing_id=listing_id,
                country_code=envelope.metadata.country_code,
            )
            if published_listing_record is None:
                raise CommandRejectedError(
                    status_code=404,
                    error_code="listing_not_found",
                    reason_code="listing_not_found",
            ...
            if published_listing_record.actor_id == envelope.metadata.actor_id:
                raise CommandRejectedError(
                    status_code=403,
                    error_code="policy_denied",
            ...
            thread = self.marketplace_repository.create_negotiation_thread(
                ...
                listing_id=listing_id,
                seller_actor_id=published_listing_record.actor_id,
        ...
        if envelope.command_name == "market.negotiations.counter":
            ...
            negotiation_thread = self.marketplace_repository.get_negotiation_thread(
                thread_id=thread_id
            )
            if negotiation_thread is None:
                raise CommandRejectedError(
                    status_code=404,
            ...
            if (
                negotiation_thread.seller_actor_id != envelope.metadata.actor_id
                or negotiation_thread.country_code != envelope.metadata.country_code
            ):
                raise CommandRejectedError(
                    status_code=403,
                    error_code="policy_denied",
            ...
            self._ensure_open_thread(negotiation_thread, thread_id=thread_id)
            negotiation_thread = self.marketplace_repository.update_negotiation_thread(
            ...
            return self._thread_result(
                envelope=envelope, thread=negotiation_thread, transition="offer_countered"
            )
        ...
        if envelope.command_name == "market.negotiations.confirm.request":
            ...
            if (
                envelope.metadata.actor_id not in self._participant_ids(negotiation_thread)
                or negotiation_thread.country_code != envelope.metadata.country_code
            ):
                raise CommandRejectedError(
                    status_code=403,
            ...
            if required_confirmer_actor_id not in self._participant_ids(negotiation_thread):
                raise CommandRejectedError(
                    status_code=403,
                    error_code="policy_denied",
            ...
            return self._thread_result(
                envelope=envelope, thread=negotiation_thread, transition="confirmation_requested"
            )
        ...
        if envelope.command_name == "market.negotiations.confirm.approve":
            ...
            if negotiation_thread.country_code != envelope.metadata.country_code:
                raise CommandRejectedError(
                    status_code=403,
                    error_code="policy_denied",
                    reason_code="thread_access_forbidden",
            ...
            if negotiation_thread.status != "pending_confirmation":
                raise CommandRejectedError(
                    status_code=409,
            ...
            if negotiation_thread.required_confirmer_actor_id != envelope.metadata.actor_id:
                raise CommandRejectedError(
                    status_code=403,
                    error_code="policy_denied",
                    reason_code="unauthorized_confirmer",
            ...
            return self._thread_result(
                envelope=envelope, thread=negotiation_thread, transition="confirmation_approved"
            )
        ...
        if envelope.command_name == "market.negotiations.confirm.reject":
            ...
            return self._thread_result(
                envelope=envelope, thread=negotiation_thread, transition="confirmation_rejected"
            )
        ...
        if envelope.command_name == "wallets.escrows.initiate":
            ...
            thread_id = str(envelope.payload["thread_id"])
            accepted_thread = self.marketplace_repository.get_negotiation_thread(thread_id=thread_id)
            ...
            if accepted_thread.status != "accepted":
                raise CommandRejectedError(
                    status_code=409,
            ...
            if envelope.metadata.actor_id not in self._participant_ids(accepted_thread):
                raise CommandRejectedError(
                    status_code=403,
                    error_code="policy_denied",
                    reason_code="thread_access_forbidden",
            ...
            if existing_escrow is None:
                escrow = self.escrow_repository.create_escrow(
                    ...
                    listing_id=accepted_thread.listing_id,
                    buyer_actor_id=accepted_thread.buyer_actor_id,
                    seller_actor_id=accepted_thread.seller_actor_id,
                    country_code=accepted_thread.country_code,
                    currency=accepted_thread.current_offer_currency,
                    amount=accepted_thread.current_offer_amount,
        ...
        if envelope.command_name == "wallets.escrows.dispute_open":
            ...
            if envelope.metadata.actor_id not in self._participant_ids_for_escrow(escrow_record) | {"system:test"}:
                raise CommandRejectedError(
                    status_code=403,
                    error_code="policy_denied",
                    reason_code="escrow_access_forbidden",
        ...
        if envelope.command_name == "climate.alerts.acknowledge":
            ...
            climate_alert = self.climate_repository.get_alert_for_actor(
                alert_id=alert_id,
                actor_id=envelope.metadata.actor_id,
                country_code=envelope.metadata.country_code,
            )
            if climate_alert is None:
                raise CommandRejectedError(
                    status_code=404,
            ...
            if climate_alert.status == "acknowledged":
                raise CommandRejectedError(
                    status_code=409,
            ...
            alert_farm_profile = self.climate_repository.get_farm_profile(
                farm_id=climate_alert.farm_id
            )
        ...
        if envelope.command_name == "climate.mrv.create":
            ...
            farm_id = str(envelope.payload["farm_id"])
            actor_farm_profile = self.climate_repository.get_farm_profile_for_actor(
                farm_id=farm_id,
                actor_id=envelope.metadata.actor_id,
                country_code=envelope.metadata.country_code,
            )
            if actor_farm_profile is None:
                raise CommandRejectedError(
                    status_code=404,
            ...
            record = self.climate_repository.create_mrv_evidence_record(
                ...
                degraded_reason_codes=degraded_reason_codes,
                summary={
                    "farm_name": actor_farm_profile.farm_name,
                    "crop_type": actor_farm_profile.crop_type,
            ...
            return {
                "mrv_evidence": _mrv_evidence_to_payload(
                    record, envelope.metadata.schema_version, actor_farm_profile
                ),
                "schema_version": envelope.metadata.schema_version,
            }
