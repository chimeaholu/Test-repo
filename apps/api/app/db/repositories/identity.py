        ...
        *,
        ...
        statement = (
            ...
            .where(
                ...
                or_(
                    func.lower(IdentitySessionRecord.display_name).like(pattern),
                    func.lower(IdentitySessionRecord.email).like(pattern),
                    func.lower(IdentitySessionRecord.actor_id).like(pattern),
                ),
            )
            .order_by(IdentitySessionRecord.display_name.asc(), IdentitySessionRecord.id.asc())
            .limit(limit)
        )
        if exclude_actor_id is not None:
            statement = statement.where(IdentitySessionRecord.actor_id != exclude_actor_id)
