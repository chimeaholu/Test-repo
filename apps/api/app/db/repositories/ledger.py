...
class EscrowRepository:
    ...
    def list_timelines_for_escrows(
        self, *, escrow_ids: list[str]
    ) -> dict[str, list[EscrowTimelineProjection]]:
        if not escrow_ids:
            return {}
        statement = (
            select(EscrowTimelineEntry)
            .where(EscrowTimelineEntry.escrow_id.in_(escrow_ids))
            .order_by(
                EscrowTimelineEntry.escrow_id.asc(),
                EscrowTimelineEntry.created_at.asc(),
                EscrowTimelineEntry.id.asc(),
            )
        )
        entries = list(self.session.execute(statement).scalars().all())
        grouped: dict[str, list[EscrowTimelineProjection]] = {escrow_id: [] for escrow_id in escrow_ids}
        for entry in entries:
            grouped.setdefault(entry.escrow_id, []).append(
                EscrowTimelineProjection(
                    escrow_id=entry.escrow_id,
                    actor_id=entry.actor_id,
                    transition=entry.transition,
