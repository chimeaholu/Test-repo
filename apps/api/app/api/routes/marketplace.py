...
def list_listings(
    ...
    if auth_context.role == "buyer":
        items: Sequence[Listing | ListingProjection] = repository.list_published_listings(
            country_code=auth_context.country_code or "GH"
        )
    else:
        items = repository.list_listings(
...
def get_listing(
    ...
    repository = MarketplaceRepository(db_session)
    if auth_context.role == "buyer":
        listing: Listing | ListingProjection | None = repository.get_published_listing(
...
def list_negotiations(
    ...
    return {
        "schema_version": get_envelope_schema_version(),
        "items": [
            _thread_payload(thread, repository.list_negotiation_messages(thread_id=thread.thread_id))
