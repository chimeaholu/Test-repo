...
async function runProject(project) {
  ...
  await signInAndGrantConsent(page, stamp, "farmer");
  await gotoPath(page, "/onboarding/consent");
  await page.getByRole("heading", { name: "Review access before the workspace opens" }).waitFor();
  const consentA11y = await assertA11ySmoke(page, "/onboarding/consent");
  const consentFile = `${project.name}-03-consent.png`;
  await page.screenshot({ path: path.join(screenshotDir, consentFile), fullPage: true });
  results.push({ project: project.name, route: "/onboarding/consent", screenshot: `screenshots/${consentFile}`, a11y: consentA11y });
  await page.locator("input[name='accepted']").check();
  await page.getByRole("button", { name: "Grant consent" }).click();
  ...
  await captureRoute(page, project.name, 11, "dispatch", "/app/cooperative/dispatch", "Coordinate dispatch, member actions, and proof checkpoints");
