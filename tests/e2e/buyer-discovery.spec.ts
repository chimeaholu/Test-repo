  ...
  test("buyer reaches the discovery shell and cannot read another actor's listing detail", async ({
    ...
    await expect(
      page.getByRole("heading", {
        name: "Review supply quickly, inspect proof, and move offers without losing context.",
      }),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("heading", {
        name: "Discover live lots without owner controls leaking into view",
      }),
    ).toBeVisible();
    await expect(page.getByText("Draft leak prevention is explicit")).toBeVisible();
