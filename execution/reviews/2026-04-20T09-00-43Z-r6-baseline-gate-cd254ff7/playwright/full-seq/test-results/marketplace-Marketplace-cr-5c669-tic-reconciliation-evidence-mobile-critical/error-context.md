...
- Name: marketplace.spec.ts >> Marketplace create and read >> farmer edits a listing and sees optimistic reconciliation evidence
- Location: tests/e2e/marketplace.spec.ts:33:7

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://127.0.0.1:3010/signin
Call log:
  - navigating to "http://127.0.0.1:3010/signin", waiting until "domcontentloaded"
...
> 253 | 
  ...
  261 |       if (
  262 |         (!message.includes("net::ERR_ABORTED") &&
  263 |           !message.includes("page crashed") &&
  264 |           !message.includes("net::ERR_INSUFFICIENT_RESOURCES")) ||
  265 |         attempt === 2
  266 |       ) {
  267 |         throw error;
  268 |       }
  ...
  272 | }
  273 | 
  274 | async function waitForInteractiveForm(page: Page, route: "/signin" | "/onboarding/consent"): Promise<void> {
  275 |   await expect(page).toHaveURL(new RegExp(`${route.replace("/", "\\/")}(\\?.*)?$`), {
  276 |     timeout: 20_000,
  277 |   });
  278 |   await expect(page.locator("form[data-interactive='true']")).toBeVisible({
  279 |     timeout: 20_000,
  280 |   });
  281 | }
  282 | 
